...
public class Conexao {
	...
	public Conexao(String bd, String local, String porta, String banco,
			String user, String senha) {
		if (bd.equals("PostgreSql")) {
			setStr_conexao("jdbc:postgresql://" + local + ":" + porta + "/"
					+ banco);
			setLocal(local);
			setSenha(senha);
			setUser(user);
			...
			if (bd.equals("MySql")) {
				setStr_conexao("jdbc:mysql://" + local + ":" + porta + "/"
						+ banco);
				setLocal(local);
				setSenha(senha);
				setUser(user);
	...
	public void conect() {
		try {
			...
			setC(DriverManager.getConnection(getStr_conexao(), getUser(),
					getSenha()));
			...
			e.printStackTrace();
			System.out.println("Conexão não realizada com sucesso.");  
		}
	}
